...
The screen-side device runs the standard one-line installer (see [`../../user/INSTALL.md`](/installation/)). The printer side needs no changes — stock QIDI firmware already exposes Moonraker on port 7125. Auto-detection picks up the Plus 4 / 3-series model via hostname or QIDI-specific macros and applies the correct preset.
